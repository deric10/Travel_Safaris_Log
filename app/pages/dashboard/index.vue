  ...
  <div class="flex-1 flex">
    ...
    <div class="bg-base-100 transition-all duration-300 " :class="{ 'w-64': isSideBarOpen, 'w-16': !isSideBarOpen }">
      <div class="flex p-2" :class="{ 'justify-center': !isSideBarOpen, 'justify-end': isSideBarOpen }">
        <Icon v-if="isSideBarOpen" name="tabler:chevron-left" size="32" @click="toggleSidebar()" />
        <Icon v-if="!isSideBarOpen" name="tabler:chevron-right" size="32" @click="isSideBarOpen = !isSideBarOpen" />
      </div>
      ...
      <div class="divider" />
      <SidebarButton :show-label="isSideBarOpen" label="Sign Out" icon="tabler:logout-2" href="" />
    </div>
    <div class="bg-amber-200 text-black flex flex-1 justify-center align-middle">
